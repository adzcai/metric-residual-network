...
class Agent(object):
    ...
    def __init__(self, args, env):
        ...
        self.actor_optim = torch.optim.Adam(
            self.actor.parameters(), lr=self.args.lr_actor
        )

        # observation/goal normalizer
        self.s_norm = Normalizer(
            size=args.dim_state, default_clip_range=self.args.clip_range
        )
    ...
    def _select_actions(self, s_tensor, g_tensor, stochastic):
        ...
        rand_a = np.random.uniform(
            low=-max_action, high=max_action, size=self.args.dim_action
        )
    ...
    def _update_normalizer(self, S, A, AG, G):
        transition = self.sample_func(S, A, AG, G, A.shape[1])
    ...
    def prefill_buffer(self):
        ...
        n_collects = (
            self.args.n_init_episodes // self.args.rollout_n_episodes // n_threads + 1
        )
        ...
        for i in range(n_episodes):
            ...
            for t in range(T):
                if uniform_random_action:
                    a = np.random.uniform(
                        low=-max_action, high=max_action, size=(dim_action,)
                    )
                else:
    ...
    def eval_agent(self):
        ...
        local_hitting_time = first_nonzero(
            total_success_rate, 1, self.args.max_episode_steps + 1
        ).mean()
        ...
        return (
            ...
            global_hitting_time / MPI.COMM_WORLD.Get_size(),
        )
    ...
    def save_model(self, stats):
        ...
        torch.save(
            sd, os.path.join(self.args.save_dir, f"{self.args.experiment_name}.pt")
        )
        if stats["successes"][-1] > self.best_success_rate:
            ...
            torch.save(
                sd,
                os.path.join(
                    self.args.save_dir, f"{self.args.experiment_name}_best.pt"
                ),
            )
