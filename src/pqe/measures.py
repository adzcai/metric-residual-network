...
class MeasureBase(nn.Module):
    def __init__(self, shape: torch.Size):
        ...
        self.shape = shape
        assert len(shape) == 2, (
            "shape should be [num_quasipartition_mixtures, num_process_per_mixture]"
        )
...
class GaussianBasedMeasure(MeasureBase):
    def __init__(self, shape: torch.Size, *, init_sigma2=1):
        ...
        self.log_sigma2 = nn.Parameter(
            torch.empty(shape).fill_(init_sigma2).log_().requires_grad_()
        )
        self.scales_net = DeepLinearNet(
            shape.numel(), shape.numel(), non_negative=True, bias=True
        )  # Sec. C.4.3
    ...
    def scale_multiple(self, *xs: torch.Tensor) -> Tuple[torch.Tensor, ...]:
        ...
        return tuple(
            scaled_x.unflatten(-1, self.shape)
            for scaled_x in self.scales_net(*[x.flatten(-2) for x in xs])
        )
